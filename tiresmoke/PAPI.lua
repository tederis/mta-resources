--[[
	Filename: PAPI.lua
	Description: Реализация частиц и предоставление API
	Author: TEDERIs <xcplay@gmail.com>
	Revision: 49 from 14.03.16
]]

local sw, sh = guiGetScreenSize ( )

XR_INTERNAL_RENDER = false
if XR_INTERNAL_RENDER then
	_G["XR_INTERNAL_MODEL"] = 1234
	_G["XR_MAX_INTERNALS"] = 60
	_G["XR_MAX_INTERNALS_PER_SHADER"] = 4
else
	_G["XR_TEST_COLOR"] = tocolor ( 255, 255, 255, 255 )
end
		...
		--if bitAnd ( self.flags, 0x40 ) > 0 then
			...
			--if bitAnd ( self.flags, 0x80000000 ) == 0x80000000 then
				while counter > 0 do
					...
					if effect.parentEffect.enableXForm ~= true then
						pos = effect.parentEffect.xform:transformPosition(pos)
					end
					...
					if effect.parentEffect.enableXForm ~= true then
						vel = effect.parentEffect.xform:transformDirection(vel)
					end
...
pDomain = {
	...
	-- Type 0 Point
		if self.type == 1 then
			return Vector3(self.p1.x, self.p1.y, self.p1.z)
		...
		elseif self.type == 10 then
			...
			return Vector3(self.center.x + radius*math.cos ( angle ), self.center.y + radius*math.sin ( angle ), self.center.z)
...
function ParticleManager:loadActions ( alist_id, xml )
	...
	for _, node in ipairs ( xmlNodeGetChildren ( xml ) ) do
		...
		if action then
			action.type = PActionEnum [ actionName ]
		
			action:load ( node )
			table.insert ( actions, action )
			
			outputDebugString ( actionName .. " loaded(" .. action.type .. ")" )
		end
	end
...
function ParticleManager:createEffect ( max_particles, parentEffect )
...
function CParticleEffect.new ( )
	...
	particleEffect.handleEffect = particleMngr:createEffect ( 1, particleEffect )
	
	-- TEST
	local x, y, z = getElementPosition ( localPlayer )
	--particleEffect.xform = makeMatrix(0, 0, 0, 0, 0, 0 )
	particleEffect.xform = Matrix()
...
function CParticleEffect:getParticlesCount ( )
	local particleMngr = getParticleManager ( )
	return particleMngr:getParticlesCount ( self.handleEffect )
end
...
function CParticleEffect:setMatrix(mat)
	self.xform = Matrix(mat)
end

local DEBUG = false

local _drawSprite = function ( up, right, pos, lt, rb, r1, r2, color, angle, def )
	...
	local spriteVec = Vector3 ( x1 - pos.x, y1 - pos.y, z1 - pos.z )
	spriteVec:normalize ( )
	local rightVec = Vector3 ( x3 - pos.x, y3 - pos.y, z3 - pos.z )
	rightVec:normalize ( )
	local faceToward = spriteVec:cross ( rightVec )
	faceToward:normalize ( )
	--	

	if XR_INTERNAL_RENDER then
		local height = getDistanceBetweenPoints3D ( x1, y1, z1, x2, y2, z2 )
		local cx, cy, cz = interpolateBetween ( x1, y1, z1, x2, y2, z2, 0.5, "Linear" )
		
		def:addInternal ( Vector3 ( cx, cy, cz ), Vector2 ( width, height ), faceToward, color )
	else
		dxDrawMaterialSectionLine3D ( 
			x1, y1, z1, x2, y2, z2,
			lt.x, lt.y, rb.x - lt.x, rb.y - lt.y,
			def.cachedShader, width, XR_TEST_COLOR,
...
local _drawSprite2 = function ( pos, dir, lt, rb, r1, r2, color, angle, def )
	...
	--
	local spriteVec = Vector3 ( ( pos.x + v2DifX ) - ( pos.x + v1NegX ), ( pos.y + v2DifY ) - ( pos.y + v1NegY ), ( pos.z + v2DifZ ) - ( pos.z + v1NegZ ) )
	spriteVec:normalize ( )
	local rightVec = Vector3 ( ( pos.x + v3NegX ) - ( pos.x + v1NegX ), ( pos.y + v3NegY ) - ( pos.y + v1NegY ), ( pos.z + v3NegZ ) - ( pos.z + v1NegZ ) )
	rightVec:normalize ( )
	local faceToward = spriteVec:cross ( rightVec )
	faceToward:normalize ( )
	--
...
function CParticleEffect:_renderParticle ( particle, lod )
	...
	if XR_INTERNAL_RENDER ~= true then
		dxSetShaderValue(def.cachedShader, "diffuse_", particle.color.r / 255, particle.color.g / 255, particle.color.b / 255, particle.color.a / 255)
	end
	
	-- Frame существует?
	--if self.enableXForm then
	...
	if def.APDefaultRotation == nil then
		if self.enableXForm then -- Флаг обычно стоит всегда
		...
		local vecToCam = Vector3 ( cx - pos.x, cy - pos.y, cz - pos.z )
		vecToCam:normalize ( )
		local upVector = Vector3 ( 0, 1, 0 )
		
		right = vecToCam:cross ( upVector )
		right:normalize ( )
		up = right:cross ( vecToCam )
		up:normalize ( )
	else
		local speed = math.sqrt ( particle.vel.x*particle.vel.x + particle.vel.y*particle.vel.y + particle.vel.z*particle.vel.z )
		if speed < 0.0000001 and UNKNOWN_FLAG_9999  then
			...
			if speed < 0.0000001 then
				...
				if not self.enableXForm then -- Флаг обычно стоит всегда
					_drawSprite2 ( pos, dir, lt, rb, r_x, r_y, particle.color, particle.rot.x,
						def
					)
				else
	...
	_drawSprite ( up, right, pos, lt, rb, r_x, r_y, particle.color, particle.rot.x, 
		def
	)
end
...
function CPEDef.new ( )
	...
	if XR_INTERNAL_RENDER then
		def.shaders = { }
		def.internalPos = { } -- float3
		def.internalSize = { } -- float2
		def.internalNormal = { } -- float3
		def.internalColor = { } -- float4
		def.internalNum = 0
		def.shaderIndex = 1
		
		local x, y, z = getElementPosition ( localPlayer )
		def.object = createObject ( XR_INTERNAL_MODEL, x, y, z )
	end
...
function CPEDef:load ( xml )
	...
	-- FOR TEST
	self.textureElement = dxCreateTexture ( "textures/" .. self.textureName )
	
	if XR_INTERNAL_RENDER then
		local shadersNum = XR_MAX_INTERNALS / XR_MAX_INTERNALS_PER_SHADER
		for i = 1, shadersNum do
			local cachedShader = dxCreateShader ( "shaders/" .. self.shaderName .. "_internal.fx" )
			dxSetShaderValue ( cachedShader, "Tex0", self.textureElement )
			engineApplyShaderToWorldTexture ( cachedShader, "fxtex" .. i - 1, self.object )
			table.insert ( self.shaders, cachedShader )
		end
	else
...
if XR_INTERNAL_RENDER then
	-- effect - CParticleEffect
	function CPEDef:clearInternalOrder ( )
		self.internalPos = { }
		self.internalSize = { }
		self.internalNormal = { }
		self.internalColor = { }
	...
	function CPEDef:addInternal ( pos, size, normal, color )
		local shadersNum = XR_MAX_INTERNALS / XR_MAX_INTERNALS_PER_SHADER
		if self.shaderIndex > shadersNum then
			return
		end
	
		self.internalNum = self.internalNum + 1
		local shaderIndex = math.ceil ( self.internalNum / XR_MAX_INTERNALS_PER_SHADER )
		if shaderIndex > self.shaderIndex then
			local shader = self.shaders [ self.shaderIndex ]
			dxSetShaderValue ( shader, "InternalPos", self.internalPos )
			dxSetShaderValue ( shader, "InternalSize", self.internalSize )
			dxSetShaderValue ( shader, "InternalNormal", self.internalNormal )
			dxSetShaderValue ( shader, "InternalColor", self.internalColor )
		...
		table.insert ( self.internalPos, pos.x )
		table.insert ( self.internalPos, pos.y )
		table.insert ( self.internalPos, pos.z )
			
		table.insert ( self.internalSize, size.x / 2 )
		table.insert ( self.internalSize, size.y / 2 )
			
		table.insert ( self.internalNormal, normal.x )
		table.insert ( self.internalNormal, normal.y )
		table.insert ( self.internalNormal, normal.z )
		
		table.insert ( self.internalColor, color.r / 255 )
		table.insert ( self.internalColor, color.g / 255 )
		table.insert ( self.internalColor, color.b / 255 )
		table.insert ( self.internalColor, color.a / 255 )
	end
	
	function CPEDef:updateInternal ( )
		local shadersNum = XR_MAX_INTERNALS / XR_MAX_INTERNALS_PER_SHADER
	
		if self.internalNum > 0 and self.shaderIndex <= shadersNum then
			local shader = self.shaders [ self.shaderIndex ]
			dxSetShaderValue ( shader, "InternalPos", self.internalPos )
			dxSetShaderValue ( shader, "InternalSize", self.internalSize )
			dxSetShaderValue ( shader, "InternalNormal", self.internalNormal )
...
--[[local x, y, z = getElementPosition ( localPlayer )
local object = createObject ( XR_INTERNAL_MODEL, x, y, z )

local cachedShader = dxCreateShader ( "shaders/blend_internal.fx" )
engineApplyShaderToWorldTexture ( cachedShader, "fxtex0", object )
...
dxSetShaderValue ( cachedShader, "InternalPos", positions )
local normals = {
	0, -1, 0,
	0, -1, 0,
	0, -1, 0,
...
dxSetShaderValue ( cachedShader, "InternalNormal", normals )
local sizes = {
	1, 1,
	1, 1,
	1, 1,
...
dxSetShaderValue ( cachedShader, "InternalSize", sizes )]]
...
addEventHandler ( "onClientResourceStart", resourceRoot,
	function ( )
		if XR_INTERNAL_RENDER then
			local col = engineLoadCOL( "models/fxmesh.col" )
			local txd = engineLoadTXD( "models/fxmesh.txd" )
			local dff = engineLoadDFF( "models/fxmesh.dff", 0 )
			 
			engineReplaceCOL ( col, XR_INTERNAL_MODEL )
			engineImportTXD ( txd, XR_INTERNAL_MODEL )
			engineReplaceModel ( dff, XR_INTERNAL_MODEL, true )
...
function unregisterEffect(effect)
