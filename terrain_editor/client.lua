...
local function getWorldHeight ( height )
	return PATCH_Z + height*MAP_SPACING_Z - HALF_ELEVATION
end
...
function xrEngine.init ( )
	...
	--xrEngine.grndTex = dxCreateTexture ( "textures/detail_grnd_earth.dds" )
...
function xrEngine.initLighting ( )
	g_Enviroment = LIP.load ( "weathers/" .. xrEngine.envName )
	xrEngine.reloadLighting ( )
end

function xrEngine.reloadLighting ( )
	xrEngine.times = { }

	-- Найдем главную секцию
	weatherSection = g_Enviroment.sect_default_weather
	
	for time, sectionName in pairs ( weatherSection ) do
		local data = g_Enviroment [ sectionName ]
		if data then
			local hour = gettok ( time, 1, 58 )
			local minutes = gettok ( time, 2, 58 )
		
			local timestamp = ( hour * 60 ) + minutes
		
			data.time = time
		
			xrEngine.times [ timestamp ] = data
		else
			outputDebugString ( "Не было найдено секции " .. sectionName )
		end
	end
end

function xrEngine.getNearestTimePoint ( timestamp, less )
	local maxTime = 0
	for time, data in pairs ( xrEngine.times ) do
		maxTime = math.max ( maxTime, time )
	end

	local value = less == true and 0 or maxTime--( 23 * 60 ) + 60

	for time, data in pairs ( xrEngine.times ) do
		if less then
			if time > value and time < timestamp then
				value = time
			end
		else
			if time < value and time > timestamp then
...
function xrEngine.getMapFromWorldPosition ( x, y )
	...
	return _mathFloor ( deltaX * mapSizeX ), _mathFloor ( deltaY * mapSizeY )
end

function xrEngine.loadWorld ( )
	xrStreamerWorld.init ( )
	
	xrEngine.initLighting ( )
	
	addEvent ( "onClientTCTEditMode", false )
	
	--bindKey ( "x", "both", xrEngine.toggleMode )
	addEventHandler ( "onClientTCTEditMode", root, xrEngine.toggleMode, false ) -- TCT integration
...
function xrEngine.toggleMode ( enabled )
	if enabled then
		...
		removeEventHandler ( "onClientCursorMove", root, xrEngine.onCursorMove )
...
function xrEngine.onRender ( )
	...
	for sector, _ in pairs ( xrStreamerWorld.activated ) do
		if sector.loaded and isElement ( sector.shader ) then
			dxSetShaderValue ( sector.shader, "L_ambient", ambr, ambg, ambb, 1 )
			dxSetShaderValue ( sector.shader, "L_hemi_color", hemir, hemig, hemib, 1 )
			dxSetShaderValue ( sector.shader, "L_sun_color", sunr, sung, sunb )
		end
	end
...
addCommandHandler ( "reloadmaps",
	function ( )
		for sector, _ in pairs ( xrStreamerWorld.activated ) do
			-- Выгружаем старые текстуры
			xrStreamerWorld.reloadSectorShader ( sector._index )
		end
		
		outputChatBox ( "Все карты обновлены" )
	end
)

addCommandHandler ( "xrshaderunload",
	function ( )
		SHADER_UNLOAD_ENABLED = not SHADER_UNLOAD_ENABLED
		if SHADER_UNLOAD_ENABLED then
			outputChatBox ( "Теперь шейдеры будут выгружаться" )
		else
			outputChatBox ( "Теперь шейдеры не будут выгружаться" )
		end
	end
)
...
sendXTGCallback = function ( stage, file )
	if stage == 1 then 
		file = fileCreate ( "heightmap.xtg" )
		if file then
			g_SaveCr = coroutine.create ( Heightfield.saveToBinary )
			coroutine.resume ( g_SaveCr, file, sendXTGCallback, 2, file )
		end
	elseif stage == 2 then
		fileSetPos ( file, 0 )
		content = fileRead ( file, fileGetSize ( file ) )
		triggerLatentServerEvent ( "onBuildXTG", 500000, resourceRoot, content )
			
		fileClose ( file )
	end
end
addEvent ( "onClientBuildXTG", true )
addEventHandler ( "onClientBuildXTG", resourceRoot,
	function ( content )
		local texture = dxCreateTexture ( "textures/heightfield.png" )
		local plain = dxGetTexturePixels ( texture )
		destroyElement ( texture )
		
		Heightfield.fillFromPlain ( plain )
		
		setTimer ( sendXTGCallback, 500, 1, 1 )
		
		--g_SmoothCr = coroutine.create ( Heightfield.smooth )
		--coroutine.resume ( g_SmoothCr, sendXTGCallback )
	end
, false )
...
function getTerrainHeight ( x, y )
	local level = PATCH_Z + Heightfield.getHeight ( x, y )*MAP_SPACING_Z - HALF_ELEVATION
