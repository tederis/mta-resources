...
--WORLD_SIZE = 6000 / SECTOR_SIZE --40 now
HALF_SECTOR_SIZE = SECTOR_SIZE / 2
MAP_STEP = 3
MAP_SIZE = SECTOR_SIZE / MAP_STEP --50 now
MAP_SPACING_Z = 1
BUILD_PATCHES = true
MAP_MAX_ELEVATION = math.floor ( 256 * MAP_SPACING_Z )
...
function xrStreamerWorld.update ( )
	...
	-- Обновляем изображения и удаляем просроченные
	local now = getTickCount ( )
	if SHADER_UNLOAD_ENABLED and now - xrStreamerWorld.imgUpdTime > 3000 then
		xrStreamerWorld.imgUpdTime = now
		
		for index, img in pairs ( xrStreamerWorld.images ) do
			local sector = xrStreamerWorld.sectors [ index ]
			-- Если сектор выгружен больше некоторого времени назад, выгружаем его текстуры
			if xrStreamerWorld.activated [ sector ] ~= true and img.lastTime ~= nil and now - img.lastTime > IMG_UNLOAD_TIME then
				xrStreamerWorld.unloadSectorShader ( index )
				break
			end
		end
	end
...
function xrStreamerWorld.setMapPixel ( x, y, level, buildSet )
	...
	if sector and xrStreamerWorld.activated [ sector ] and sector.loaded then
		...
		for i = 1, #involvedSectors do
			...
			involved:updateVertex ( x - mapX, y - mapY, (level * MAP_SPACING_Z) - HALF_ELEVATION )
...
function xrStreamerWorld.loadSectorShader ( imgIndex )
	local imgData = xrStreamerWorld.images [ imgIndex ]
	if imgData then
		--imgData.refs = imgData.refs + 1
		imgData.lastTime = nil
		
		return imgData.shader
	else
		local shader = dxCreateShader ( "shader.fx" )
	
		local baseTex = dxCreateTexture ( "sectors/sector" .. imgIndex .. ".png" )
		if baseTex then
			dxSetShaderValue ( shader, "TexBase", baseTex )
		else
			outputDebugString ( "Не могу загрузить sector" .. imgIndex .. ".png" )
		end
		local maskTex = dxCreateTexture ( "sectors/sector" .. imgIndex .. "_mask.png" )
		if maskTex then
			dxSetShaderValue ( shader, "TexDet", maskTex )
		else
			outputDebugString ( "Не могу загрузить sector" .. imgIndex .. "_mask.png" )
		end
		
		xrStreamerWorld.images [ imgIndex ] = {
			--refs = 1,
			base = baseTex,
			mask = maskTex,
			shader = shader
		...
		xrStreamerWorld.imagesNum = xrStreamerWorld.imagesNum + 1
		
		--outputDebugString ( "Шейдер " .. imgIndex .. " загружен" )
		
		return shader, true
	end
end

function xrStreamerWorld.unloadSectorShader ( imgIndex )
	if SHADER_UNLOAD_ENABLED ~= true then
		return
	end
	...
	if imgData then
		local now = getTickCount ( )
		--imgData.refs = imgData.refs - 1
		--if imgData.refs <= 0 then
		if imgData.lastTime ~= nil and now - imgData.lastTime > IMG_UNLOAD_TIME then
			if isElement ( imgData.base ) then
				destroyElement ( imgData.base )
			end
			if isElement ( imgData.mask ) then
				destroyElement ( imgData.mask )
			end
			destroyElement ( imgData.shader )
			xrStreamerWorld.images [ imgIndex ] = nil
			
			xrStreamerWorld.imagesNum = xrStreamerWorld.imagesNum - 1
			
			--outputDebugString ( "Шейдер " .. imgIndex .. " выгружен" )
		else
			imgData.lastTime = now
		end
	end
end

function xrStreamerWorld.reloadSectorShader ( imgIndex )
	if SHADER_UNLOAD_ENABLED ~= true then
		return
	end
	...
	if imgData then
		...
		local baseTex = dxCreateTexture ( "sectors/sector" .. imgIndex .. ".png" )
		if baseTex then
			dxSetShaderValue ( imgData.shader, "TexBase", baseTex )
		else
			outputDebugString ( "Не могу загрузить sector" .. imgIndex .. ".png" )
		end
		local maskTex = dxCreateTexture ( "sectors/sector" .. imgIndex .. "_mask.png" )
		if maskTex then
			dxSetShaderValue ( imgData.shader, "TexDet", maskTex )
		else
			outputDebugString ( "Не могу загрузить sector" .. imgIndex .. "_mask.png" )
		end
	end
...
function xrStreamerSector:load ( )
	...
	for valueName, texture in pairs ( xrEngine.maskTextures ) do
		dxSetShaderValue ( self.shader, valueName, texture )
	end
	
	engineApplyShaderToWorldTexture ( self.shader, "_Texture_", self.base )
	engineApplyShaderToWorldTexture ( self.shader, "_Texture_", self.lod )
	
	if fileExists ( "sectors/sector" .. self._index .. ".png" ) then
		self.backTex = dxCreateTexture ( "sectors/sector" .. self._index .. ".png" )
		dxSetShaderValue ( self.shader, "TexBase", self.backTex )
		
		self.maskTex = dxCreateTexture ( "sectors/sector" .. self._index .. "_mask.png" )
		dxSetShaderValue ( self.shader, "TexDet", self.maskTex )
	else
		outputDebugString ( "Не существует текстуры для сектора " .. self._index .. "!", 2 )
	end]]
	
	local shader, justCreated = xrStreamerWorld.loadSectorShader ( self._index )
	-- Если шейдер только что создали
	if justCreated then
		for valueName, texture in pairs ( xrEngine.maskTextures ) do
			dxSetShaderValue ( shader, valueName, texture )
		end
	end
	
	engineApplyShaderToWorldTexture ( shader, "_Texture_", self.base )
	engineApplyShaderToWorldTexture ( shader, "_Texture_", self.lod )
	
	self.shader = shader -- Для обновления
...
function xrStreamerSector:unload ( )
	if self.loaded then
		...
		if isElement ( self.backTex ) then
			destroyElement ( self.backTex )
			destroyElement ( self.maskTex )
		end]]
		
		xrStreamerWorld.unloadSectorShader ( self._index )
...
addEventHandler ( "onClientSectorResponse", resourceRoot,
	function ( fileType, index, content, arg )
		...
		-- Base texture
		elseif fileType == FILE_TX then
			local file = fileCreate ( "sectors/sector" .. index .. ".png" )
			fileWrite ( file, content )
			fileClose ( file )
			
		-- Mask texture
		elseif fileType == FILE_MASK then
			local file = fileCreate ( "sectors/sector" .. index .. "_mask.png" )
			fileWrite ( file, content )
			fileClose ( file )
		end
